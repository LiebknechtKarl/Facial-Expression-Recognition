...
def parse_args():
    ...
    parser.add_argument('--raf_path', type=str, default='data', help='Raf-DB dataset path.')
...
def run_training():
    ...
    val_dataset = datasets.ImageFolder(f'{args.raf_path}/valid', transform = data_transforms_val)    
    ...
    for epoch in tqdm(range(1, args.epochs + 1)):
        ...
        with torch.no_grad():
            ...
            # with open('Epoch_train.csv', 'a', newline='', encoding='utf-8') as csvfile:
                # writer = csv.writer(csvfile, quoting=csv.QUOTE_NONE, quotechar='')
                # # writer.writerow([str(epoch) , '\t' ,       str(acc),'\t',     str(balanced_acc),'\t',     str(running_loss)    ])
                # epoch_msg = str(epoch) + '\t' +       str(acc)+'\t'+     str(balanced_acc)+'\t'+     str(running_loss)
                # # writer.writerow(str(epoch) + '\t' +       str(acc)+'\t'+     str(balanced_acc)+'\t'+     str(running_loss)    )
            with open('Epoch_train.csv', "a") as f:

                f.write(str(epoch) + '\t' +       str(acc)+'\t'+     str(balanced_acc)+'\t'+     str(running_loss)+'\n')
            ...
            if acc == best_acc:
                ...
                torch.save({'iter': epoch,
                            ...
                            os.path.join('checkpoints', str(TIME_STAMP) + ".pth"))
                ...
                # plt.savefig(os.path.join('checkpoints', "rafdb_epoch"+str(epoch)+"_acc"+str(acc)+"_bacc"+str(balanced_acc)+".png")) # TIME_STAMP
                plt.savefig(os.path.join('checkpoints', str(TIME_STAMP) + ".png")) # TIME_STAMP
